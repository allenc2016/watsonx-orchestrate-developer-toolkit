...
FastAPI and [Langflow](https://www.langflow.org/) to create a chat completion service that integrates with 
Google Gemini model.
...
- **Integration with AI Models**: It provides an example that supports uses Google Gemini to provide a travel iternary.
...
3. **Create the Code Engine Application:**
   ...
   - Click **Specify build details**:
     - **SSH secret:** None
     - **Branch name:** main
     - **Context directory:** `external_agent/examples/langflow_python`
     - Click **Next**
...
5. **Test the Application:**
   ...
   - Append `/docs` to the end of the URL path to view a formatted API page.
     - Example: `https://wxo-agent-test1-xyz-south.codeengine.appdomain.cloud/docs`

### Step 2: Register the New Endpoint as an External Agent

1. **In IBM watsonx orchestrate Web UI:**
   - Create a new Agent - you can give this any name, e.g. "Travel Assistant"
   - Now, we need to add the external Agent as a collaborator. Scroll down to the "Agents" section.
   - Select **Add agent**.
   - Select **Import** from the options.
   - Choose **External agent** at the top of the dialog.

2. **Enter Details:**
   - **Display Name:** e.g., "Travel Planning Agent"
   - **Description:** Enter a description of capabilities, for instance `You're a helpful agent that will propose a travel itinerary for a trip to a city for a given number of days`
   - **API Key:** Enter your API key.
   - **Service Instance URL:** Use the Test URL with `/chat/completions` appended.
     - Example: `https://wxo-agent-test1-xyz.codeengine.appdomain.cloud/chat/completions`

![alt text](./register_travel_planner.png "Example of registering an external agent to IBM watsonx Orchestrate")
